...
#se ejecuta 4 veces para crear iterar entre los 4 puntos de la mesa
#¿esto para qué es?
#¿esto es al azar?
margeX=400
margeY=300
...
#se el archivo json para guardarlo en data
with open('camera.json', 'r') as f:
    data = json.load(f)

#se itera en data para crear variables globales
for k,v in data.items():
    globals()[k]=v
    print("primera data:", data)

#imprimir los valores de data
#esto es a qué cámara se refiere
print("Camera Data:", data)

#intentar abrir el archivo .json del punto 1
try:
    #una vez abierto carga los datos en la variable "data"
    #ESTO YA ESTÁ PRE-CONFIGURADO ASÍ QUE PUEDE SER LOS PRIMEROS 4 CIRCULOS QUE APARECEN
    #???????????????
    with open('data.json', 'r') as f:
        data = json.load(f)

    #se crean variables globales para los valores de "data"
    #esto parece que es para pasar el proyecto a la cámara
    #????????
    for k,v in data.items():
        globals()[k]=np.array(v)
        print("segunda data:", data)
